...
const DigitalTwinsPage: React.FC = () => {
  ...
  const getViewToken = async (fileId: number, existingToken: string | null): Promise<string | null> => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/api/bimface/getviewid?fileId=${fileId}&token=cn-937d5634-69e4-4543-9ea6-d2f2545ff3bc`);
    ...
    for (const model of models) {
      ...
      if ((model.status === 'success' || model.status === 'completed') && !model.viewtoken && model.file_id) {
        try {
          ...
          const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/api/bimface/getviewid?fileId=${model.file_id}&token=cn-937d5634-69e4-4543-9ea6-d2f2545ff3bc`);
          const data = await response.json();
      ...
      if (model.status === 'waiting' || model.status === 'processing' || model.status === 'started') {
        try {
          ...
          const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/api/bimface/status/${model.file_id}`);
